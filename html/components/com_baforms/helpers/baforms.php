...
abstract class baformsHelper 
{
    ...
    protected static function restoreHTML($formSettings, $element, $submissionsOptions, $elements)
    {
        ...
        $array = array('textInput', 'textarea', 'email', 'htmltext', 'map');
        if (!in_array($settings[2], $array) && (self::$about->tag != 'pro' || !isset(self::$state->data))) {
            $path = '';
        }
        if ($path) {
            include $path;
    ...
    public static function getFormsFooter($id)
    {
        ...
        if (self::$about->tag != 'pro') {
            ...
            $html .= '<a href="http://www.balbooa.com/joomla-forms">Joomla Forms</a> makes it right. Balbooa.com</p>';
        }
    ...
    public static function drawHTMLPage($id)
    {
        ...
        self::$state = self::checkFormsActivation();
        $form = self::getForm($id);
        $payments = self::getPayment($id);
        ...
        $url = JUri::root().'index.php?option=com_baforms&view=form';
        $language = JFactory::getLanguage();
        $language->load('com_baforms', JPATH_ADMINISTRATOR);
        ...
        if ($popup['display_popup'] == 1 && self::$about->tag == 'pro' && isset(self::$state->data)) {
            if ($popup['button_type'] != 'link') {
                $html .= '<div class="btn-' .$popup['button_position']. '">';
                $html .= "<input type='button' value='".htmlspecialchars($popup['button_lable'], ENT_QUOTES);
        ...
        if ($saveContinue->enable == 1 && self::$about->tag == 'pro' && isset(self::$state->data)) {
            $token = self::getToken();
        ...
        if ($method != '' && $submissionsOptions['display_total'] == 1 && self::$about->tag == 'pro' && isset(self::$state->data)) {
            $html .= ' ba-payment';
        }
        ...
        foreach ($columns as $column) {
            ...
            if (trim($column[1]) == 'spank' && self::$about->tag == 'pro' && isset(self::$state->data)) {
                if (count($column) > 6) {
        ...
        if ($submissionsOptions['display_cart'] == 1 && $submissionsOptions['display_total'] == 1
            && self::$about->tag == 'pro' && isset(self::$state->data)) {
        ...
        if ($submissionsOptions['display_total'] == 1 && self::$about->tag == 'pro' && isset(self::$state->data)) {
        ...
        if ($payments->multiple_payment == 1 && $submissionsOptions['display_total'] == 1
            ...
            if (!empty($payments->yandex_shopId) && !empty($payments->yandex_scid)) {
                $html .= '<option value="form.yandex">Yandex Kassa</option>';
            }
            if (!empty($payments->barion_poskey) && !empty($payments->barion_email)) {
                $html .= '<option value="form.barion">Barion</option>';
            }
            if (!empty($payments->payu_pl_pos_id) && !empty($payments->payu_pl_second_key)) {
                $html .= '<option value="form.payupl">Payu.pl</option>';
            }
        ...
        } else if ($method == 'yandex' && $submissionsOptions['display_total'] == 1
            && self::$about->tag == 'pro' && isset(self::$state->data)) {
            $html .= '<input type="hidden" name="task" value="form.yandex">';
        } else if ($method == 'barion' && $submissionsOptions['display_total'] == 1
            && self::$about->tag == 'pro' && isset(self::$state->data)) {
            $html .= '<input type="hidden" name="task" value="form.barion">';
        } else if ($method == 'payupl' && $submissionsOptions['display_total'] == 1
            && self::$about->tag == 'pro' && isset(self::$state->data)) {
            $html .= '<input type="hidden" name="task" value="form.payupl">';
        } else {
        ...
        if ($capt != '0') {
            ...
            foreach ($doc->_style as $key => $value) {
            ...
            foreach ($doc->_scripts as $key => $value) {
                ...
                if (isset($value['defer']) && !empty($value['defer'])) {
                    $str .= ' defer';
                }
                if (isset($value['async']) && !empty($value['async'])) {
                    $str .= ' async';
                }
                $str .= '></script>';
            }
            foreach ($doc->_script as $key => $value) {
                ...
                $str .= '<script type="'.$key.'">'.$value.'</script>';
            }
            ...
            $html .= $str;
            if ($capt == 'recaptcha') {
                $captchaStr = $captcha->display($capt, $capt, 'g-recaptcha');
                preg_match_all('/data-callback="[^"]+/', $captchaStr, $matches, PREG_SET_ORDER);
                if (empty($matches[0])) {
                    preg_match_all('/data-callback="/', $captchaStr, $matches, PREG_SET_ORDER);
                }
                if (!empty($matches[0])) {
                    $match = $matches[0][0];
                    if (strpos($match, '=')) {
                        $captchaStr = str_replace($match, 'data-callback="verifyFormsRecaptcha', $captchaStr);
                    } else {
                        $captchaStr = str_replace($match, 'data-callback="verifyFormsRecaptcha"', $captchaStr);
                    }
                }
        ...
        if ($saveContinue->enable == 1 && isset($_GET['bf_token']) && self::$about->tag == 'pro' && isset(self::$state->data)) {
