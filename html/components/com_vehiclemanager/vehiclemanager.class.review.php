...
class mosVehicleManager_review extends JTable
{
    ...
    function check() {
        ...
        if ( $this->rating == 0) {
            JFactory::getApplication()->enqueueMessage(_VEHICLE_MANAGER_INFOTEXT_JS_REVIEW_RATING , 'error');
            return false;
        }
        if (trim($this->title) == "") {
            JFactory::getApplication()->enqueueMessage(_VEHICLE_MANAGER_INFOTEXT_JS_REVIEW_TITLE , 'error');
            $this->setError();
            return false;
        }
        if (trim($this->comment) == "") {
            JFactory::getApplication()->enqueueMessage(_VEHICLE_MANAGER_INFOTEXT_JS_REVIEW_COMMENT , 'error');
            return false;
        }
        return true;
    }
