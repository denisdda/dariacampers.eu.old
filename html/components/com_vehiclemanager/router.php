...
function VehicleManagerBuildRoute(&$query)
{
    ...
    if (isset($query['Itemid'])  && $query['Itemid'] !== 0  )
    {
        ...
        if( isset($query['view']) && $query['view'] == "my_vehicles" ) {

          $user_name = isset($menu->getItem($query['Itemid'])->query['name']) ? $menu->getItem($query['Itemid'])->query['name'] : '';
          if(isset($name) && $name != "" ) $query['name'] = $user_name;
        }
   ...
   // print_r("VehicleManagerBuildRoute start2:") ;
   // print_r($query) ;
   // print_r($params) ;
    ...
    { //check component
        ...
        if (isset($query['catid']) && $segments[1] !== "search" && $segments[1] !== "search_vehicle"  )
        {
            if ($query['catid'] > 0)
            {
                $sql_query = "SELECT vmc.id, vmc.name, vmc.title, vc.idcat, vmc.parent_id " .
                ...
                if (isset($row))
                {
                    ...
                    $segments[] = JFilterOutput::stringURLUnicodeSLug($row->title);
                    unset($query['catid']);
                }
            } else
            {
        ...
        if (isset($query['id']))
        {
            ...
            if (isset($row))
            {
                $temp_title = JFilterOutput::stringURLUnicodeSLug($row->vtitle);
                if (isset($temp))
                {
                    $sql_query = "SELECT name, title FROM #__vehiclemanager_main_categories WHERE id = " . $row->catid;
                    $db->setQuery($sql_query);
                    $row = $db->loadObject();
                    if (isset($row))
                    {
...
function VehicleManagerParseRoute($segments)
{
    ...
    //print_r($item) ;
    if( !is_numeric($segments[0] ) ){
        array_unshift($segments, $item->id );
    } else if( $item->id != $segments[0] )
    {
        $menu->setActive($segments[0]);
        $vars['Itemid'] = $segments[0];

        //below part not work for community builde !!!????        
        // array_unshift($segments, $item->id );
    ...
    } elseif (@$segments[1] == "owner_vehicles")
    {
        ...
        //if ( (int)$segments[2] == 0 ) echo ":2222222222222222:";

        if (isset($segments[2]) and ( (int)$segments[2] == 0 )   )
        {
         //echo ":1111111111111111:".$vars['Itemid'];
            $vars['name'] = $segments[2];
            if (isset($segments[3]))
            {
                $vars['start'] = $segments[3];
        ...
        // $menu = new JTableMenu($db);
        // $menu->load($vars['Itemid']);
        // $params = new JRegistry;
        // $params->loadString($menu->params);
         //$vars['name'] = $params->get('username', "");
