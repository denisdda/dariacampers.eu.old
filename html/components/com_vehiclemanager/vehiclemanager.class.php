...
class mosVehicleManager extends JTable
{
    ...
    function check()
    {
        ...
        if(!$vehiclemanager_configuration['price']['string']){
            $this->price = floatval(preg_replace('/[\s,]/', '', $this->price));
        }
        if (trim($this->vehicleid) == "")
        {
            JFactory::getApplication()->enqueueMessage( _VEHICLE_MANAGER_ADMIN_INFOTEXT_JS_EDIT_VEHICLEID_CHECK, 'error');
            return false;
        }
        ...
        // {
        //     JFactory::getApplication()->enqueueMessage( _VEHICLE_MANAGER_ADMIN_INFOTEXT_JS_EDIT_VEHICLEID, 'error');
        //     return false;
        // }
        return true;
    }
    ...
    // 'deleteVehicle()' - the main function to delete one or few vehicles from front-end and admin. This function colls when delete one or few vehicles from front-end and from admin:
    function deleteVehicle() {
        global $database, $my, $option, $Itemid, $mosConfig_absolute_path, $vehiclemanager_configuration, $acl;

        $bid[] = $this->id;

        $database->setQuery("DELETE FROM #__vehiclemanager_rent WHERE fk_vehicleid = ".$this->id." ");
        $database->execute() ;

        $database->setQuery("DELETE FROM #__vehiclemanager_review WHERE fk_vehicleid = ".$this->id."");
        $database->execute();

        $database->setQuery("DELETE FROM #__vehiclemanager_categories WHERE iditem = ".$this->id."");
        $database->execute();

        $database->setQuery("DELETE FROM #__vehiclemanager_feature_vehicles WHERE fk_vehicleid = ".$this->id."");
        $database->execute();

            $query = "select associate_vehicle from #__vehiclemanager_vehicles where id = ".$this->id;
            ...
            if(!empty($assocVehicleObj)){
                ...
                if(!empty($stringIdWhereChange)){
                    $query =
                      "update #__vehiclemanager_vehicles set associate_vehicle ='$newAssocSerialize' " .
              ...
              $sql = "SELECT src FROM #__vehiclemanager_video_source WHERE fk_vehicle_id = ".$this->id;
              ...
              $sql = "DELETE FROM #__vehiclemanager_video_source WHERE (fk_vehicle_id = $this->id )";
              $database->setQuery($sql);
              $database->execute();

              $sql = "SELECT src FROM #__vehiclemanager_track_source WHERE fk_vehicle_id =". $this->id;
              $database->setQuery($sql);
              ...
              $sql = "DELETE FROM #__vehiclemanager_track_source WHERE (fk_vehicle_id = $this->id )";
              $database->setQuery($sql);
              $database->execute();
                ...
                if ($this->edok_link != '' )
                {
                    $path = $mosConfig_absolute_path . '/components/com_vehiclemanager/edocs';
                    $edok_file_inf = pathinfo($this->edok_link);
                    $edok_filename = $edok_file_inf['filename'];
                    $edok_basename = $edok_file_inf['basename'];
                    $edoc_file = $path . '/' . $edok_basename;
                    if ($edok_filename > 20) {
                        if ( file_exists( $edoc_file ) ) unlink( $edoc_file );
                    }
                }
              ...
              if ($this->image_link != '' )
              {
               ...
               $del_photo_mask_inf = pathinfo($this->image_link);
               $del_photo_mask_type = '.' . $del_photo_mask_inf['extension'];
               $del_photo_mask = basename($this->image_link, $del_photo_mask_type);
               // if( strlen($del_photo_mask) < 20 ) {
               if( strlen($del_photo_mask) > 20 ) {
            ...
            $database->setQuery("SELECT fk_vehicleid,thumbnail_img, main_img FROM #__vehiclemanager_photos WHERE fk_vehicleid = $this->id ");
            ...
            for ($i = 0; $i < count($del_photos); $i++) {
              if ($del_photos[$i]->main_img != '')
              {
               ...
               if(strlen($del_photo_mask) < 20 ) continue ;
                ...
                $database->setQuery("DELETE FROM #__vehiclemanager_photos WHERE fk_vehicleid = " . $del_photos[$i]->fk_vehicleid );
                $database->execute();

                $database->setQuery("DELETE FROM #__vehiclemanager_vehicles WHERE id =" . $del_photos[$i]->fk_vehicleid );
                $database->execute();
            }

            $database->setQuery("DELETE FROM #__vehiclemanager_photos WHERE fk_vehicleid = $this->id ");
            $database->execute();

            $database->setQuery("DELETE FROM #__vehiclemanager_vehicles WHERE id = $this->id");
            $database->execute();

        /*$database->setQuery("delete FROM #__vehiclemanager_mls_for_delete where $this->vehicleid = mls ");
        $database->execute(); */
    }
