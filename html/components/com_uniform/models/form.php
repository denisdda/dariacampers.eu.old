...
class JSNUniformModelForm extends JModelItem
{
	...
	private function _getActionForm($formAction, $formData, &$return)
	{
		switch($formAction)
		{
			...
			case 4:
			case 5:
				$return->actionForm = $formAction == 4 ? "message" : 'modal';
				$return->actionFormData = $formData;
				break;
		}
	}
	...
	private function _fieldJson($post, $fieldType, $fieldIdentifier)
	{
		...
		else
		{
            ...
            {
                return $data ? json_encode(array_filter($data, 'strlen'), JSON_UNESCAPED_UNICODE) : "";
            }
		}
	}
	...
	public function save($post)
	{
		...
		if ((int) $dataForms->form_captcha
			&& (! (int) $dataForms->form_hide_captcha_if_logged_in || ! (int) $user->get('id')))
		{
			...
			elseif ($dataForms->form_captcha == 2)
			{
			...
			elseif ($dataForms->form_captcha == 3)
			{
				...
				if (is_file(JPATH_PLUGINS . '/captcha/recaptcha_invisible/recaptcha_invisible.php'))
				{
					...
					// Check the invisible captcha.
					try
					{
						$result = $plugin->onCheckAnswer();
		...
		if (!$validationForm)
		{
			...
			// If form action is custom message, replace placeholders with submitted field values.
			if (in_array($dataForms->form_post_action, array('4', '5'))
				&& preg_match_all('/\{\$([^\}]+)\}/i', $dataForms->form_post_action_data, $matches, PREG_SET_ORDER))
			{
				foreach ($matches as $match)
				{
					$dataForms->form_post_action_data = str_replace($match[0], @$dataContentEmail[$match[1]], $dataForms->form_post_action_data);
				}
			}
...
//		$regex = '/^([\p{L}\.\-\d]+)@([\p{L}\-\.\d]+)(\.[\p{L}])((\.[\p{L}]{2,63})+)$/';
		if (!empty($listEmail) && is_array($listEmail) && count($listEmail))
		{
			...
			if ($dataTemplates->template_notify_to == 1)
			{
				// Append form location to email message.
				if (!empty($_SERVER['HTTP_REFERER']))
				{
					$body .= sprintf(JText::_('JSN_UNIFORM_FORM_LOCATION'), $_SERVER['HTTP_REFERER']);
				}
			}
