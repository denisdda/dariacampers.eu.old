...
$cV = explode(' ', $vVersion);
$GLOBALS['currentVersion'] = $currentVersion = substr($cV[0],0,3);
$GLOBALS['currentnumber'] = $currentnumber = $cV[1];
...
function updateVehConfigurationVersion() {
    ...
    $vehiclemanager_configuration['release']['version'] = (string)$xml->version;
    $vehiclemanager_configuration['release']['date'] = (string)$xml->creationDate;
...
function update_to_version_3_5(){
  ...
  echo "Creating table for vehiclemanager orders details... ";
  $query = "CREATE TABLE IF NOT EXISTS `#__vehiclemanager_orders_details` (
      ...
      FOREIGN KEY (`fk_order_id`) REFERENCES #__vehiclemanager_orders(`id`),
      FOREIGN KEY (`fk_vehicle_id`) REFERENCES #__vehiclemanager_vehicles(`id`),
      FOREIGN KEY (`fk_user_id`) REFERENCES #__users(`id`),
      FOREIGN KEY (`fk_vehicle_vtitle`) REFERENCES #__vehiclemanager_vehicles(`vtitle`),
      PRIMARY KEY (`id`)) ENGINE=MyISAM  DEFAULT CHARSET=utf8;";
  $database->setQuery($query);
  $database->execute();

  if (array_search($database->getPrefix() . "vehiclemanager_orders_details", $database->getTableList(), true))
    echo "[<span style='color: #0061CC; font-weight: bold;'>Ok!</span>]<br />";
  else
    exit;

  echo "Creating table for vehiclemanager video source... ";
  $query = "CREATE TABLE IF NOT EXISTS #__vehiclemanager_video_source (
  ...
  if (array_search($database->getPrefix() . "vehiclemanager_video_source", $database->getTableList(), true))
    echo "[<span style='color: #0061CC; font-weight: bold;'>Ok!</span>]<br />";
  else
    exit;

  echo "Creating table for vehiclemanager track source... ";
  $query = "CREATE TABLE IF NOT EXISTS #__vehiclemanager_track_source (
    `id` int(11) unsigned NOT NULL auto_increment,
    `fk_vehicle_id`int(11) DEFAULT NULL,
    `sequence_number` int(11) DEFAULT NULL,
    `src` varchar(255) DEFAULT NULL,
  ...
  if (array_search($database->getPrefix() . "vehiclemanager_track_source", $database->getTableList(), true))
    echo "[<span style='color: #0061CC; font-weight: bold;'>Ok!</span>]<br />";
  else
    exit;

  $query = "ALTER  TABLE #__vehiclemanager_const
      ADD UNIQUE (`const`)";
  $database->setQuery($query);
  $database->execute();

  $query = "ALTER  TABLE #__vehiclemanager_languages
      ADD UNIQUE (`lang_code`)";
  $database->setQuery($query);
  $database->execute();

  //added in 3.3.1
  $query = "ALTER TABLE #__vehiclemanager_orders
      CHANGE `data` `order_date` DATETIME NULL DEFAULT NULL,
      CHANGE `email` `usr_email` VARCHAR(255) NOT NULL DEFAULT '',
      CHANGE `name` `usr_name` VARCHAR(255) NOT NULL DEFAULT '',
      ADD COLUMN `order_currency_code` VARCHAR( 255 ) NOT NULL ,
      ADD COLUMN `order_price` INT( 11 ) NULL AFTER `payer_status`,
      ADD COLUMN `fk_vehicle_vtitle` VARCHAR( 255 ) NOT NULL ,
      ADD FOREIGN KEY (`fk_vehicle_vtitle`) REFERENCES #__vehiclemanager_vehicles (`vtitle`),
      ADD FOREIGN KEY (`fk_vehicle_id`) REFERENCES #__vehiclemanager_vehicles (`id`),
      ADD FOREIGN KEY (`fk_user_id`) REFERENCES #__users (`id`),
...
function update_to_version_3_7(){
  ...
  $query = "ALTER TABLE #__vehiclemanager_orders_details
      ADD COLUMN `paypal_paykay` VARCHAR( 255 ) NOT NULL";
  $database->setQuery($query);
  $database->execute();

  $query = "ALTER  TABLE #__vehiclemanager_photos
...
function update_to_version_3_9(){
  ...
  $query = "CREATE TABLE IF NOT EXISTS `#__vehiclemanager_users_wishlist` (
            `id` int(11) NOT NULL auto_increment,
  ...
  if (array_search($database->getPrefix() . "vehiclemanager_users_wishlist", $database->getTableList(), true))
    echo "[<span style='color: #0061CC; font-weight: bold;'>Ok!</span>]<br />";
  else
    exit;
...
function update_to_version_4_1 () {
    global $database;
    $query = "ALTER TABLE `#__vehiclemanager_vehicles` CHANGE `price` `price` VARCHAR(255) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT ''";
    $database->setQuery($query);
    $database->execute();
        echo "[<span style='color: #0061CC; font-weight: bold;'>Ok!</span>]<br />";
...
function update_to_version_5_0 () {
    ...
    $query = "ALTER TABLE `#__vehiclemanager_rent_request` ADD `user_phone` VARCHAR(250) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT '' AFTER `user_email`";
    $database->setQuery($query);
    $database->execute();
        echo "Add column user_phone to the table vehiclemanager_rent_request - [<span style='color: #0061CC; font-weight: bold;'>Ok!</span>]<br />";

    $query = "ALTER TABLE `#__vehiclemanager_rent` ADD `user_phone` VARCHAR(250) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT '' AFTER `user_email`";
    $database->setQuery($query);
    $database->execute();
        echo "Add column user_phone to the table vehiclemanager_rent - [<span style='color: #0061CC; font-weight: bold;'>Ok!</span>]<br />";

    $query = "ALTER TABLE `#__vehiclemanager_vehicles` MODIFY `maker` varchar(50) NOT NULL DEFAULT '';";
    $database->setQuery($query);
    $database->execute();
        echo "Change column maker from vehiclemanager_vehicles on varchar(50) - [<span style='color: #0061CC; font-weight: bold;'>Ok!</span>]<br />";

    $query = "ALTER TABLE `#__vehiclemanager_orders_details` ADD `fk_request_id` int(11) DEFAULT NULL AFTER `paypal_paykay`";
    $database->setQuery($query);
    $database->execute();
            echo "Add column fk_request_id to the table vehiclemanager_orders_details - [<span style='color: #0061CC; font-weight: bold;'>Ok!</span>]<br />";
...
function update_to_version_5_5() {
    ...
    $query = "CREATE TABLE IF NOT EXISTS  `#__vehiclemanager_api` (
            `id` int(11) NOT NULL AUTO_INCREMENT,
...
function com_install2(){
  global $database, $mosConfig_absolute_path, $mosConfig_live_site, $vehiclemanager_configuration,$currentVersion,$currentnumber;
  ...
  echo "[<span style='color: #0061CC; font-weight: bold;'>Ok!</span>]<br>Checking XSL extension... ";
  ...
  echo "[<span style='color: #0061CC; font-weight: bold;'>Ok!</span>]<br>Checking PHP-Xml extension... ";
  ...
  echo "[<span style='color: #0061CC; font-weight: bold;'>Ok!</span>]<br>Checking CURL extension... ";
  ...
  echo "[<span style='color: #0061CC; font-weight: bold;'>Ok!</span>]<br>Checking GD extension... ";
  ...
  echo "[<span style='color: #0061CC; font-weight: bold;'>Ok!</span>]<br><br>Preparation for database installation... ";

  //******************   database ******************

  $tableList = $database->getTableList();
  echo '<br/>... cheking database...';
  ...
  if (array_search($table_prefix . 'vehiclemanager_vehicles', $tableList)){
    ...
    $vm = '';
  if ($vm){
    echo '<br/>Database already exist!<br/>';
    switch ($version['version']) {
      case 2.1:
          ...
          $query = "ALTER TABLE #__vehiclemanager_vehicles
                    ADD COLUMN `associate_vehicle` varchar(255) default null";
          $database->setQuery($query);
          $database->execute();
    ...
    updateVehConfigurationVersion();
    $vers = explode(' ', $vehiclemanager_configuration['release']['version']);
    $currentVersion = substr($vers[0],0,3);
    $currentnumber = $vers[1];
    $query = "UPDATE #__vehiclemanager_version
              SET version = '" . $currentVersion . "', number = '" . $currentnumber .
              "' WHERE id = 2";
            $database->setQuery($query);
            $database->execute();
    echo '<br>End upgrading database';
  }else{
    // If database doesnt exists, create it
    echo '<br/>Install database start: <br/>';
    $query = "DROP TABLE IF EXISTS `#__vehiclemanager_mime_types`, `#__vehiclemanager_const_languages`,
        ...
        `#__vehiclemanager_buying_request`, `#__vehiclemanager_review`,
        `#__vehiclemanager_categories`, `#__vehiclemanager_photos`, `#__vehiclemanager_version`,
        `#__vehiclemanager_vehicles`, `#__vehiclemanager_main_categories`, `#__vehiclemanager_orders`, `#__vehiclemanager_users_wishlist`, `#__vehiclemanager_api` ";
    $database->setQuery($query);
    $database->execute();
    ...
    echo "Creating table for vehiclemanager videos... ";
    ...
    echo "Creating table for vehiclemanager track ... ";
    ...
    echo "Creating table for const languages ... ";
    ...
    echo "Creating table for __vehiclemanager_api ... ";
    $query = "CREATE TABLE IF NOT EXISTS  `#__vehiclemanager_api` (
            `id` int(11) NOT NULL AUTO_INCREMENT,
            `service` varchar(255) NOT NULL,
            `key` text NOT NULL,
    ...
    //var_dump($database->getTableList());
    if (array_search($database->getPrefix() . "vehiclemanager_api", $database->getTableList(), true))
        echo "[Ok]<br />";
    else
        exit;

    echo "Install database end!<br/>";
  } // End of creation new database
  //****************************************** add language and constant ***************
  vmLittleThings::loadConstVechicle();
  
  vm_add_new_features_category_constant();
  vm_add_new_features_constant();
  //****************************************** end language and constant ***************
  ...
  if (!isset($vehiclemanager)){
    mosVehicleManagerImportExport :: importVehiclesXML($mosConfig_absolute_path .
     "/administrator/components/com_vehiclemanager/exports/sample_data.xml", null,true);
    echo "Sample data loaded<br/>";
  }
  //****************************************** add featured constant ***************
  vm_add_new_features_constant();
  //****************************************** end featured constant ***************
  //******************   end database ******************

  if (!$is_warning)
    echo "<p style='color: #0061CC; font-weight: bold;'>Import sample data - OK</p>";
  ...
  <center>
    <table width="100%" border="0">
      <tr>
        <td>
          <strong style='color: #0f0;'>Installation VehicleManager (Joomla component for creating sites for selling or renting vehicles) successfull!</strong><br/>
          <br/>
      ...
      <tr>
        <td>
          <code>Installation: <font color='#0f0'>successfull</font></code>
        </td>
      </tr>
    </table>
  </center>
...
function loadDataMime(){
  ...
  $query = "TRUNCATE TABLE  `#__vehiclemanager_mime_types`";
    $database->setQuery($query);
    $database->execute();
