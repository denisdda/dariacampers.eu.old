...
jQuery(document).ready(function(){
    ...
    var formFields = new Array(),
        ...
        delay = '',
        google_sheets = JSON.parse(jQuery('#jform_google_sheets').val()),
        ...
        acymailingLists = JSON.parse(jQuery('#jform_acymailing_lists').val()),
        acymailingFields = JSON.parse(jQuery('#jform_acymailing_fields_map').val()),
        acymLists = JSON.parse(jQuery('#jform_acym_lists').val()),
        acymFields = JSON.parse(jQuery('#jform_acym_fields_map').val());

    if (google_sheets.spreadsheet && sheets && sheets[google_sheets.spreadsheet] && !sheets[google_sheets.spreadsheet].worksheets) {
        var data = {
                spreadsheet: sheets[google_sheets.spreadsheet]
            },
            xhr = new XMLHttpRequest();
        xhr.open('POST', 'index.php?option=com_baforms&view=form&task=form.getWorkSheets', true);
        xhr.onreadystatechange = function () {
            if (this.readyState === 4) {
                sheets[google_sheets.spreadsheet].worksheets = JSON.parse(this.responseText);
                if (google_sheets.worksheet && sheets && !sheets[google_sheets.spreadsheet].worksheets[google_sheets.worksheet].columns) {
                    data = {
                        spreadsheet : sheets[google_sheets.spreadsheet],
                        worksheet : sheets[google_sheets.spreadsheet].worksheets[google_sheets.worksheet]
                    };
                    xhr = new XMLHttpRequest();
                    xhr.open('POST', 'index.php?option=com_baforms&view=form&task=form.getWorkSheetsColumns', true);
                    xhr.onreadystatechange = function () {
                        if (this.readyState === 4) {
                            sheets[google_sheets.spreadsheet].worksheets[google_sheets.worksheet].columns = JSON.parse(this.responseText);
                            matchSheets();
                        }
                    };
    ...
    {
        jQuery('.google-sheet-spreadsheets, .google-sheet-worksheets').find('option').each(function(ind){
            if (this.value != '') {
                jQuery(this).remove();
            }
        });
        jQuery('.merge-google-sheets-fields').empty();
        var str = '';
        for (var key in sheets) {
            str = '<option value="'+sheets[key].id+'"';
            if (sheets[key].id == google_sheets.spreadsheet) {
                str += ' selected';
                jQuery('.google-sheet-worksheets').removeAttr('disabled');
            }
            str += '>'+sheets[key].title+'</option>';
            jQuery('.google-sheet-spreadsheets').append(str);
            if (sheets[key].id == google_sheets.spreadsheet) {
                for (ind in sheets[key].worksheets) {
                    str = '<option value="'+sheets[key].worksheets[ind].id+'"';
                    if (sheets[key].worksheets[ind].id == google_sheets.worksheet) {
                        str += ' selected';
                    }
                    str += '>'+sheets[key].worksheets[ind].title+'</option>';
                    jQuery('.google-sheet-worksheets').append(str);
                    if (sheets[key].worksheets[ind].id == google_sheets.worksheet && sheets[key].worksheets[ind].columns) {
                        var columns = sheets[key].worksheets[ind].columns;
                        for (var i = 0; i < columns.length; i++) {
                            str = '<div class="google-sheets-fiels"><label>'+columns[i];
                            str += '</label><select class="form-fields" data-field="'+columns[i]+'">';
                            str += jQuery('.google-sheet-spreadsheets option[value=""]')[0].outerHTML;
                            formFields.forEach(function(item){
                                str += '<option value="'+item.id+'"';
                                if (item.id == google_sheets.columns[columns[i]]) {
                                    str += ' selected';
                                }
                            ...
                            str += '</select></div>';
                            jQuery('.merge-google-sheets-fields').append(str);
                        }
                    }
                }
            }
        ...
        if (key) {
            jQuery('.google-sheet-spreadsheets').removeAttr('disabled');
        }
    }

    jQuery('.google-sheet-spreadsheets').on('change', function(){
        google_sheets.spreadsheet = this.value;
        google_sheets.worksheet = '';
        google_sheets.columns = {};
        matchSheets();
        jQuery('#jform_google_sheets').val(JSON.stringify(google_sheets));
        if (google_sheets.spreadsheet && sheets && sheets[google_sheets.spreadsheet] && !sheets[google_sheets.spreadsheet].worksheets) {
            var data = {
                    spreadsheet: sheets[google_sheets.spreadsheet]
                },
    ...
    jQuery('.google-sheet-worksheets').on('change', function(){
        google_sheets.worksheet = this.value;
        google_sheets.columns = {};
        matchSheets();
        jQuery('#jform_google_sheets').val(JSON.stringify(google_sheets));
        if (google_sheets.worksheet && sheets && !sheets[google_sheets.spreadsheet].worksheets[google_sheets.worksheet].columns) {
    ...
    jQuery('.merge-google-sheets-fields').on('change', 'select', function(){
        google_sheets.columns[this.dataset.field] = this.value;
        jQuery('#jform_google_sheets').val(JSON.stringify(google_sheets));
    });
    ...
    jQuery('.google-sheets-integration').on('click', function(){
        jQuery('#google-sheets-integration-dialog').modal();
    });

    jQuery('.apply-google-sheets-code').on('click', function(event){
        event.preventDefault();
        if (this.dataset.enable == 'false') {
            return false;
        } else {
            this.dataset.enable = 'false';
        }
        var $this = this;
        jQuery.ajax({
            type:"POST",
            dataType:'text',
            url:"index.php?option=com_baforms&view=form&task=form.createSheetsToken",
            data:{
                code: document.getElementById('google_sheets_code').value,
            },
            success: function(msg){
                $this.dataset.enable = '';
                if (msg && msg != 'SHEETS_INVALID_TOKEN') {
                    sheets = JSON.parse(msg);
                    jQuery('#google-sheets-integration-dialog .mailchimp-message').hide();
                    matchSheets();
                } else if (msg == 'SHEETS_INVALID_TOKEN') {
                    var text = 'Invalid Authentication Code'
                    jQuery('#google-sheets-integration-dialog .mailchimp-message').show().text(text);
                }
            }
        });
    });
    ...
    jQuery('.acymailing-integration').on('click', function(){
        jQuery('.acymailing-name select option:not([value=""])').remove();
        jQuery('.acymailing-lists option').each(function(){
            if (acymailingLists.array && acymailingLists.array.indexOf(this.value) != -1) {
                this.selected = true;
            }
        });
        formFields.forEach(function(item, i , arr){
            jQuery('.match-acymailing select:not([data-field="email"])').each(function(){
                var str = '<option value="'+item.id+'"';
                if (item.id == acymailingFields[this.dataset.field]) {
                    str += ' selected';
                }
                str += '>'+item.label+'</option>';
        ...
        emailFields.forEach(function(item, i , arr){
            var str = '<option value="'+item.id+'"';
            if (item.id == acymailingFields.email) {
                str += ' selected';
            }
            str += '>'+item.label+'</option>';
            jQuery('.match-acymailing select[data-field="email"]').append(str);
        });
        jQuery('#acymailing-integration-modal').modal();
    });

    jQuery('.acymailing-name select').on('change', function(){
        acymailingFields[this.dataset.field] = this.value;
        jQuery('#jform_acymailing_fields_map').val(JSON.stringify(acymailingFields));

    });

    jQuery('.acymailing-lists').on('change', function(){
        var array = new Array();
        ...
        acymailingLists.array = array;
        jQuery('#jform_acymailing_lists').val(JSON.stringify(acymailingLists));
    });

    jQuery('.acym-integration').on('click', function(){
        jQuery('.acym-name select option:not([value=""])').remove();
        jQuery('.acym-lists option').each(function(){
            if (acymLists.array && acymLists.array.indexOf(this.value) != -1) {
                this.selected = true;
            }
        });
        formFields.forEach(function(item, i , arr){
            jQuery('.match-acym select:not([data-field="email"])').each(function(){
                var str = '<option value="'+item.id+'"';
                if (item.id == acymFields[this.dataset.field]) {
                    str += ' selected';
                }
                str += '>'+item.label+'</option>';
                jQuery(this).append(str);
        ...
        emailFields.forEach(function(item, i , arr){
            var str = '<option value="'+item.id+'"';
            if (item.id == acymFields.email) {
                str += ' selected';
            }
            str += '>'+item.label+'</option>';
            jQuery('.match-acym select[data-field="email"]').append(str);
        });
        jQuery('#acym-integration-modal').modal();
    });

    jQuery('.acym-name select').on('change', function(){
        acymFields[this.dataset.field] = this.value;
        jQuery('#jform_acym_fields_map').val(JSON.stringify(acymFields));

    });

    jQuery('.acym-lists').on('change', function(){
        var array = new Array();
        jQuery(this).find('option').each(function(){
            if (this.selected) {
                array.push(this.value);
        ...
        acymLists.array = array;
        jQuery('#jform_acym_lists').val(JSON.stringify(acymLists));
    });
        ...
        jQuery('#content-section .droppad_item').each(function(){
            if (jQuery(this).find('.ba-email').length > 0) {
                ...
                formFields.push(obj);
                       ...
                       jQuery(this).find('.ba-dropdown, .ba-checkMultiple').length > 0 ||
                       jQuery(this).find('.ba-date, .ba-selectMultiple, .ba-chekInline').length > 0) {
                options = jQuery(this).find(' > .ba-options').val().split(';');
                if (options[0]) {
                    label = options[0];
                } else {
    ...
    jQuery('.mailchimp-connect').on('click', function(){
        ...
        jQuery.ajax({
            ...
            success: function(msg){
                var msg = JSON.parse(msg);
                if (!msg.data) {
                    jQuery('#mailchimp-integration-dialog .mailchimp-message').show().text(msg.message);
                    jQuery('.mailchimp-select-list option').each(function(){
                        if (jQuery(this).val() != '') {
                            jQuery(this).remove();
                        }
                ...
                if (msg.message) {
                    ...
                    msg = JSON.parse(msg);
                    if (msg.status) {
                        jQuery('#mailchimp-integration-dialog .mailchimp-message').show().text(msg.title);
                        jQuery('.mailchimp-select-list option').each(function(){
                            if (jQuery(this).val() != '') {
                                jQuery(this).remove();
                            }
                        ...
                        jQuery('#mailchimp-integration-dialog .mailchimp-message').hide();
                    }
                    var len = msg.lists.length,
                        str ='';
                    for (var i = 0; i < len; i++) {
    ...
    function checkPayment()
    {
        ...
        jQuery('.yandex, .barion, .payupl').hide();
        if (value == 'paypal') {
            ...
            jQuery('.webmoney, .paypal-login').hide();
            jQuery('.yandex').show();
            jQuery('#jform_payment_environment').show().prev().show();
            jQuery('#jform_payment_environment').nextAll().show();
            jQuery('.skrill, .ccavenue').hide();
            ...
            jQuery('.stripe').hide();
            jQuery('.mollie').hide();
        } else if (value == 'barion') {
            jQuery('.barion').show();
            jQuery('#jform_payment_environment').show().prev().show();
            jQuery('#jform_payment_environment').nextAll().hide();
            jQuery('.skrill, .ccavenue, .webmoney, .paypal-login').hide();
            jQuery('.stripe, .mollie, .custom-payment, .payu-biz, .payu, .2checkout').hide();
    ...
    */
    ...
    jQuery('#stripe-image-dialog').each(function(){
        setTimeout(function(){
            var iframe = window.frames['stripe-image-dialog'];
            iframe.jQuery('[onclick*=".modal.in"]').each(function(){
                var onclick = jQuery(this).attr('onclick');
                jQuery(this).attr('onclick', onclick.replace('.modal.in', '#stripe-image-dialog'))
            });
        }, 2000);
    });
    ...
    jQuery('#item-aply').on('click', function(event){
        event.preventDefault();
