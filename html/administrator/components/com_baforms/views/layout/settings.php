...
?>
            ...
            <div id="form-submission">
                ...
                <label><?php echo JText::_('SAVE_SUBMISSIONS') ?></label>
                <input type="hidden" name="jform[save_submissions]" value="0">
                <?php echo baformsHelper::defaultCheckboxes('save_submissions', $this->form); ?>
<?php
            if ($this->about->tag == 'pro' && isset($formsState->data)) {
?>
...
?>
                <label>
                    <span>
                        <?php echo JText::_('SAVE_AND_CONTINUE') ?>
                    </span>
                    <span class="ba-tooltip">
                        <?php echo JText::_('SAVE_AND_CONTINUE_TOOLTIP'); ?>
                    </span>
                </label>
                <input type="hidden" name="jform[save_continue]" value="0">
                <?php echo $this->form->getInput('save_continue'); ?>
<?php
            }
?>
                <label>
...
<?php
            if ($this->about->tag == 'pro' && isset($formsState->data)) {
?>
                <a href="#" class="email-subject subject-link">
                    +<?php echo JText::_('PERSONALIZE_SUBJECT'); ?>
                </a>
                <label><?php echo JText::_('ATTACH_UPLOADED_FILES') ?></label>
                <input type="hidden" name="jform[attach_uploaded_files]" value="0">
                <?php echo baformsHelper::defaultCheckboxes('attach_uploaded_files', $this->form); ?>
<?php
            }
?>
...
<?php
            if ($this->about->tag == 'pro' && isset($formsState->data)) {
?>
                <a class="reply-subject subject-link" href="#">
                    +<?php echo JText::_('PERSONALIZE_SUBJECT'); ?>
                </a>
<?php
            }
...
<?php
            if ($this->about->tag == 'pro' && isset($formsState->data)) {
?>
                <a class="reply-body subject-link" href="#">
                    +<?php echo JText::_('PERSONALIZE_BODY'); ?>
                </a>
<?php
            }
?>
            ...
            <div id="payment" style="display: none;">
                ...
                <div class="yandex">
                    <label>ShopId</label>
                    <?php echo $this->form->getInput('yandex_shopId'); ?>
                    <label>Scid</label>
                    <?php echo $this->form->getInput('yandex_scid'); ?>
                </div>
                <div class="barion">
                    <label>POSKey</label>
                    <?php echo $this->form->getInput('barion_poskey'); ?>
                    <label>Barion Email</label>
                    <?php echo $this->form->getInput('barion_email'); ?>
                </div>
                <div class="payupl">
                    <label>POS ID</label>
                    <?php echo $this->form->getInput('payu_pl_pos_id'); ?>
                    <label>Second key</label>
                    <?php echo $this->form->getInput('payu_pl_second_key'); ?>
                </div>
