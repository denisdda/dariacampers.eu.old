...
class baformsModelForm extends JModelAdmin
{
    ...
    {
        $checkAcymailing = baformsHelper::checkAcymailing();
        $fields = array();
        if (!empty($checkAcymailing)) {
            $db = JFactory::getDbo();
            $query = $db->getQuery(true)
                ->select('namekey, fieldname')
                ->where('namekey <> '.$db->quote('html'))
    ...
    public function getAcymFields()
    {
        $checkAcym = baformsHelper::checkAcym();
        $fields = array();
        if (!empty($checkAcym)) {
            $db = JFactory::getDbo();
            $query = $db->getQuery(true)
    ...
    {
        $sheets = $this->getSheetsToken();
        require_once JPATH_ROOT.'/components/com_baforms/libraries/google-sheets/baSheets.php';
        $baSheets = new baSheets();
        $sheets->authUrl = $baSheets->getAuthentication();
        if (!empty($sheets->code)) {
            $sheets->tables = $baSheets->getSpreadsheet($sheets->accessToken);
        } else {
            $sheets->tables = new stdClass();
        }

        return $sheets;
    }

    public function getSheetsToken()
    {
        $db = JFactory::getDbo();
        $query = $db->getQuery(true);
        $query->select('`key`')
    ...
    public function getWorkSheets()
    {
        ...
        require_once JPATH_ROOT.'/components/com_baforms/libraries/google-sheets/baSheets.php';
        $baSheets = new baSheets();
        $worksheets = $baSheets->getWorkSheets($sheets->accessToken, $spreadsheet['title']);
    ...
    public function getWorkSheetsColumns()
    {
        ...
        require_once JPATH_ROOT.'/components/com_baforms/libraries/google-sheets/baSheets.php';
        $baSheets = new baSheets();
        $columns = $baSheets->getWorkSheetsColumns($sheets->accessToken, $spreadsheet['title'], $worksheet['title']);
    ...
    public function createSheetsToken()
    {
        ...
        $key = $db->loadResult();
        $sheets = json_decode($key);
        if (!empty($code) && $code != $sheets->code) {
            require_once JPATH_ROOT.'/components/com_baforms/libraries/google-sheets/baSheets.php';
            $baSheets = new baSheets();
            ...
            if ($token == 'SHEETS_INVALID_TOKEN') {
                ...
                $obj = new stdClass();
                $obj->key = json_encode($sheets);
                $obj->service = 'google_sheets';
                $db->updateObject('#__baforms_api', $obj, 'service');
                $sheets = $baSheets->getSpreadsheet($token);
                $key = json_encode($sheets);
