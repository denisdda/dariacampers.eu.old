...
class JSNUniformHelper
{
				...
				if (!empty($submission->$key))
				{
					...
					if ($jsonName)
					{
						...
						if (!empty($jsonName->first) || !empty($jsonName->last) || !empty($jsonName->suffix))
						{
						    ...
						    $contentField = implode(' ', (array) $jsonName);
						}
						else
						{
							$contentField = '';
	 ...
	 */
	public static function checkRecaptchaPlugin($plugin = 'recaptcha')
	{
		$db = JFactory::getDBO();
		$db->setQuery($db->getQuery(true)->from('#__extensions')->select('*')->where('element = ' . $db->quote($plugin) . ' AND folder = "captcha" AND type = "plugin"'));
	...
	public static function generateHTMLPages($formId, $formName, $formType = "", $topContent = "", $bottomContent = "", $showTitle = false, $showDes = false, $isModule = false)
	{
		...
		$loadBootstrap = isset($loadBootstrap->value) ? $loadBootstrap->value : '';
		$stylesheets = array();
		...
		if ($loadBootstrap)
		{
			if ((string) $loadBootstrap == '1')
			{
				$loadBootstrap = 'v2.3.2';
			}
			$document->addStyleSheet(JSN_UNIFORM_ASSETS_URI . "/3rd-party/bootstrap/{$loadBootstrap}/css/bootstrap.min.css");
		}
		...
		if ($items)
		{
			...
			if (isset($cConfig->form_autocomplete_form))
			{
			    if ((int) $cConfig->form_autocomplete_form)
			    {
			        $autoComplete = 'on';
			    }
			}
			...
			if (!$formType)
			{
				...
				$html .= '<div class="jsn-uniform jsn-master" data-form-name="' . $formName . '" id="jsn_form_' . $items->form_id . '"><div class="' . self::getBootstrapPrefix() . '">';
				$html .= $topContent;

				// Generate HTML code for thank you modal if necessary.
				if ($items->form_post_action == '5')
				{
					$html .= '<div class="jsn-uniform-preview-modal jsn-uniform-thank-you-modal modal hide fade" tabindex="-1" id="jsn_uniform_modal_message_form_' . $formName . '" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
				...
				$html .= "<form data-current-date='{$now}' name='form_{$formName}' data-ajaxupload='0' id='form_{$formName}' action=\"" . 'JSN_UNIFORM_ROOT_URL' . '/index.php?option=com_uniform&amp;view=form&amp;task=form.save&amp;form_id=' . $items->form_id . "\" method=\"post\" class=\"form-validate {$classForm} \" enctype=\"multipart/form-data\" autocomplete=\"{$autoComplete}\">";
			...
			$html .='<div class="control-group '. $showTotalMoneyClass .'">
				...
				if ($i + 1 == count($formPages))
				{
					...
					if ((int) $items->form_captcha
						&& (! (int) $items->form_hide_captcha_if_logged_in || ! (int) $user->get('id')))
					{
						if ($items->form_captcha == 1)
						{
							...
							if (version_compare(@$params->version, '2.0', '>='))
							{
								...
								$captchaId = "recaptcha_for_{$formName}_{$i}";
								$html .= '<script src="https://www.google.com/recaptcha/api.js?render=explicit&hl=' . JFactory::getLanguage()->getTag() . '" async defer></script>';
								...
								$html .= "<script type='text/javascript'>
									(function {$captchaId}_init() {
									    if (window.grecaptcha && grecaptcha.render) {
									        var elm = document.getElementById('{$captchaId}');
									        ...
									        setTimeout({$captchaId}_init, 100);
									    }
									})();
								</script>";
							...
							{
								if (strpos($k, 'https://www.google.com/recaptcha/api.js') !== false)
								{
									unset($doc->_scripts[$k]);
								}
								elseif (strpos($k, '/media/plg_captcha_recaptcha/js/recaptcha.min.js') !== false)
								{
									unset($doc->_scripts[$k]);
								}
							}
						}
						elseif ($items->form_captcha == 2)
						{
						...
						elseif ($items->form_captcha == 3)
						{
							// Load the Invisible reCAPTCHA plugin.
							if (is_file(JPATH_PLUGINS . '/captcha/recaptcha_invisible/recaptcha_invisible.php'))
							{
								require_once JPATH_PLUGINS . '/captcha/recaptcha_invisible/recaptcha_invisible.php';

								// Instantiate the Invisible reCAPTCHA plugin.
								$dispatcher = JEventDispatcher::getInstance();
								...
								// Render the invisible captcha.
								$captchaId = "invisible_recaptcha_for_{$formName}_{$i}";

								$html .= '<script src="https://www.google.com/recaptcha/api.js?render=explicit&hl=' . JFactory::getLanguage()->getTag() . '" async defer></script>';
								$html .= $plugin->onDisplay(null, $captchaId, 'g-invisible-recaptcha');
								$html .= "<script type='text/javascript'>
									(function {$captchaId}_init() {
									    if (window.grecaptcha && grecaptcha.render) {
											var elm = document.getElementById('{$captchaId}');
											if (isNaN(parseInt(elm.getAttribute('recaptcha-widget-id')))) {
												window.{$captchaId}_callback = function() {
												    return new Promise(function(resolve, reject) {
														// State that invisible recaptcha is resolved.
														elm = document.getElementById('{$captchaId}');
			...
			$resetCustomClass = !empty($formSettings->form_btn_reset_custom_class) ? $formSettings->form_btn_reset_custom_class : "";
			$nextCustomClass = !empty($formSettings->form_btn_next_custom_class) ? $formSettings->form_btn_next_custom_class : "";
			$prevCustomClass = !empty($formSettings->form_btn_prev_custom_class) ? $formSettings->form_btn_prev_custom_class : "";
			$htmlBtnReset = "";
			if (!empty($formSettings->form_state_btn_reset_text) && $formSettings->form_state_btn_reset_text == "Yes")
			{
				$htmlBtnReset = '<button type="reset" class="' . $btnResetStyle . ' reset ' . $resetCustomClass . '" style="display:inline-block">' . JText::_($btnReset) . '</button>';
			}else{
                $htmlBtnReset = '<button type="reset" class="' . $btnResetStyle . ' reset ' . $resetCustomClass . '" style="display:none">' . JText::_($btnReset) . '</button>';
			}
			$html .= '<div class="form-actions">
									<div class="' . $btnPosition . '">
									    <button type="button" class="' . $btnPrevStyle . ' prev ' . $prevCustomClass . ' hide">' . JText::_($btnPrev) . '</button>
									    <button type="button" class="' . $btnNextStyle . ' next ' . $nextCustomClass . ' hide">' . JText::_($btnNext) . '</button>
									    ' . $htmlBtnReset . '
			...
			if (!$formType)
			{
				...
				$html .= "<input type=\"hidden\" name=\"ajax_upload_file\" value=\"0\" />";
				$html .= "<input type=\"hidden\" name=\"use_payment_gateway\" value=\"{$use_payment_gateway}\" />";
				$html .= "<input type=\"hidden\" name=\"list_choosen_field\" value=\"\" />";
				$html .= "<input type=\"hidden\" name=\"form_post_action\" value=\"{$postAction}\" />";
	...
	public static function getBootstrapPrefix()
	{
		$loadBootstrap = self::getDataConfig('load_bootstrap_css');
		$loadBootstrap = isset($loadBootstrap->value) ? $loadBootstrap->value : '';
		$bootstrapPrefix = 'jsn-bootstrap';
		...
		if ($loadBootstrap)
		{
			...
			$bootstrapPrefix .= ' jsn-bootstrap' . preg_replace('/^v(\d+)\.\d+\.\d+$/', '\\1', $loadBootstrap);
		}
