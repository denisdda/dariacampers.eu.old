	...
	public function display($tpl = null)
	{
		...
		if (empty($this->_item->form_id))
		{
			...
			foreach ($this->_fromConfig as $formConfig)
			{
				...
				if (in_array($this->formAction, array('4', '5')) && $formConfig->name == 'form_action_message')
				{
					$this->formActionData = $formConfig->value;
				}
			}
...
// 			include_once JPATH_ROOT . "/administrator/components/com_jce/classes/parameter.php";
		...
		elseif (JPluginHelper::isEnabled('editors', 'tinymce') == true)
		{
			...
			JSNHtmlAsset::addScript(JUri::root(true) . '/media/editors/tinymce/plugins/importcss/plugin.min.js');
		...
		// Re-load language files.
		//$lang = JFactory::getLanguage();

		//$lang->load('com_uniform', JPATH_SITE, null, true);
		//$lang->load('com_uniform', JPATH_ADMINISTRATOR, null, true);
